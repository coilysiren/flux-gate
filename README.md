...
| `assemble_run_report(run_id, weapon_id, clearance_threshold)` | Build per-weapon `RiskReport` + `Clearance`; also persists confirmed-failure findings to the cross-run store as a side effect | Orchestrator |
